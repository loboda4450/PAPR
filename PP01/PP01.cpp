...
std::tuple<bool, std::string> ThreadFunctionWithArgs(int32_t WaitMilliSeconds) {
    std::thread::id ThreadID = std::this_thread::get_id();
    ...
    fmt::print("THRD ID={} FINISH\n", ThreadID);
    std::string RetVal = fmt::format("THRD ID={} START and wait {}ms\n", ThreadID, WaitMilliSeconds);
    return std::make_tuple(EXIT_SUCCESS, RetVal);
}
...
int32_t Example03() {
    ...
    while (true) //come on, use true instead of 1..
    {
        std::future_status Status = Future.wait_for(std::chrono::milliseconds(500));
...
int32_t Example04() {
    constexpr int32_t NumThreads = 10;
    ...
    for (int32_t ThreadCnt = 0; ThreadCnt < NumThreads; ThreadCnt++) {
        auto Wrapper = std::bind(ThreadFunctionWithArgs, 3000 + ThreadCnt);
        std::packaged_task<int32_t()> PackagedTask(Wrapper);
...
#else
    constexpr int32_t NumThreads = 10000; //on Linux/Unix use 10000 threads
#endif
